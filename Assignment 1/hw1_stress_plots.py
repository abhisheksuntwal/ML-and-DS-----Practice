...
def structuredpreceptron(stress_x_train, stress_y_train, types, max_iter, r, eta):
    ...
    for i in xrange(0, max_iter, 2):
        ...
        accuracy = training_correct / float(training_total) * 100
        # output.write("Training Total:=%i Training Correct=%i Training Accuracy=%f \n" % (training_total, training_correct, accuracy))


    # mlt.plot(accuracy_training)
    # mlt.ylabel('Training Accuracy')
    # mlt.xlabel('Iterations')
    # mlt.show()
    print "finished"
    return weights, training_total, training_correct

def testing(final_weights, stress_x_train, stress_y_train, types, r):
    count = 0
...
def main():
    ...
    types = ["unary", "pair", "triple", "quad"]
    final_weights = [0] * len(types)
    print("main")
    ...
    #             final_weights, training_total, training_correct = structuredpreceptron(stress_x_train, stress_y_train, "unary", i, j, k)
    #             testing_total, testing_correct = testing(final_weights, stress_x_test, stress_y_test, "unary", j)
    ...
    # output.close()
    for j in R:
        final_weights, training_total, training_correct = structuredpreceptron(stress_x_train, stress_y_train, "unary",
                                                                               20, j, 0.01)
